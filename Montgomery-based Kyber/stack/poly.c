...
void poly_basemul_montgomery(poly* r, const poly* a, const poly* b) {
	...
	for (i = 0; i < KYBER_N / 4; i++) {
		basemul_OP(&r->coeffs[4 * i], &a->coeffs[4 * i], &b->coeffs[4 * i], zetas[64 + i]);
		basemul_OP(&r->coeffs[4 * i + 2], &a->coeffs[4 * i + 2], &b->coeffs[4 * i + 2], -zetas[64 + i]);
	}
}
...
void poly_frombytes_mul(poly* r, const poly* b, const unsigned char* a) 
{
	...
	for (i = 0; i < KYBER_N / 4; i++) {
		...
		basemul_OP(&r->coeffs[4 * i], &b->coeffs[4 * i], &temp[0], zetas[64 + i]);
		basemul_OP(&r->coeffs[4 * i + 2], &b->coeffs[4 * i + 2], &temp[2], -zetas[64 + i]);
	}
}
...
void poly_frombytes_mul_acc(poly* r, const poly* b, const unsigned char* a) 
{
	...
	for (i = 0; i < KYBER_N / 4; i++) {
		...
		basemul_OP(&temp[0], &b->coeffs[4 * i], &temp[0], zetas[64 + i]);
		basemul_OP(&temp[2], &b->coeffs[4 * i + 2], &temp[2], -zetas[64 + i]);
...
void poly_basemul_acc(poly* r, const poly* a, const poly* b) {
	...
	for (i = 0; i < KYBER_N / 4; i++)
	{
		basemul(&res[0], &a->coeffs[4 * i], &b->coeffs[4 * i], zetas[64 + i]);
		basemul(&res[2], &a->coeffs[4 * i + 2], &b->coeffs[4 * i + 2], -zetas[64 + i]);
